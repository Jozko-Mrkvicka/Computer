...
for (row = 1 : numOfRows)
    ...
    switch (dtype)
        case c.BYTE
            if ((c.CONST_DATA_START > (addr - 1)) || ((c.CONST_DATA_START + c.CONST_DATA_SIZE - 1) < (addr - 1)))
        ...
        case c.WORD
            if ((c.CONST_DATA_START > (addr - 1)) || ((c.CONST_DATA_START + c.CONST_DATA_SIZE - 1) < (addr - 0)))
                error('### PREPROCESSOR ERROR: Address of a constant points outside of the data ROM!! (Name = %s, Address = 0x%04X) ###\n', name, addr - 1);
            end
        ...
        case c.TEXT
            ...
            if ((c.CONST_DATA_START > (addr - 1)) || ((c.CONST_DATA_START + c.CONST_DATA_SIZE - 1) < (addr - 1 + stringSize)))
                error('### PREPROCESSOR ERROR: Address of a constant points outside of the data ROM!! (Name = %s, Address = 0x%04X) ###\n', name, addr - 1);
            end
