label = ...
{
	'LABEL'
};
for (idx = 1:size(label))
	eval([char(label(idx)),'  = bitor(c.LABEL_SRC_PREFIX,  idx);']);
	eval([char(label(idx)),'_ = bitor(c.LABEL_DEST_PREFIX, idx);']);
end
c.LBL_CNT = idx;


SourceCode = ...
[
				MOVL		r1			hAA						... % Assign data to store/load.
				MOVU		r1			hBB						...
																...
				MOVL		r2			h00						... % Clear register.
				MOVU		r2			h00						...
																...
				MOVL		r3			hFF						... % Set register to some value.
				MOVU		r3			hFF						...
																...
				MOVL		r0			lsb(c.RAM_START + 0)	... % Assign memory address.
				MOVU		r0			msb(c.RAM_START + 0)	...
				STOREU		m(r0)		r1						... % Store register to memory.
																...
				MOVL		r0			lsb(c.RAM_START + 1)	... % Assign memory address.
				MOVU		r0			msb(c.RAM_START + 1)	...
				STOREL		m(r0)		r1						... % Store register to memory.
																...
				LOADU		r2			m(r0)					... % Expected value: r2 == 0xBB00
				LOADU		r3			m(r0)					... % Expected value: r3 == 0xBBFF
																...
	LABEL_		JMP			LABEL								...
];

